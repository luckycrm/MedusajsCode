...
interface CliInput {
  ...
  readonly mcHome: Option.Option<string>;
...
const CliEnvConfig = Config.all({
  mode: Config.string("MCCODE_MODE").pipe(
    Config.option,
  ...
  noBrowser: Config.boolean("MCCODE_NO_BROWSER").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  authToken: Config.string("MCCODE_AUTH_TOKEN").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  bootstrapFd: Config.int("MCCODE_BOOTSTRAP_FD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  autoBootstrapProjectFromCwd: Config.boolean("MCCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  logWebSocketEvents: Config.boolean("MCCODE_LOG_WS_EVENTS").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
    ...
    Effect.gen(function* () {
      ...
      const baseDir = yield* resolveBaseDir(
        Option.getOrUndefined(
          resolveOptionPrecedence(
            input.mcHome,
            Option.fromUndefinedOr(env.mcHome),
            Option.flatMap(bootstrapEnvelope, (bootstrap) =>
              Option.fromUndefinedOr(bootstrap.mcHome),
            ),
          ),
        ),
      );
...
const mcHomeFlag = Flag.string("home-dir").pipe(
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to MCCODE_LOG_WS_EVENTS).",
  ),
  Flag.withAlias("log-ws-events"),
  Flag.optional,
);

export const mcCli = Command.make("mc", {
  mode: modeFlag,
  port: portFlag,
  host: hostFlag,
  mcHome: mcHomeFlag,
  ...
  Command.withDescription("Run the MedusaJS Code server."),
