...
function isInsideWorkspaceRoot(workspaceRoot: string, candidatePath: string): boolean {
...
export async function listWorkspaceDirectory(
  ...
  const absoluteDirectoryPath = relativePath
    ? path.resolve(workspaceRoot, relativePath)
    : workspaceRoot;

  if (!isInsideWorkspaceRoot(workspaceRoot, absoluteDirectoryPath)) {
    throw new Error(`Directory is outside workspace: ${absoluteDirectoryPath}`);
  }

  const stats = await fs.stat(absoluteDirectoryPath);
  if (!stats.isDirectory()) {
    throw new Error(`Path is not a directory: ${absoluteDirectoryPath}`);
  }

  const dirents = await fs.readdir(absoluteDirectoryPath, { withFileTypes: true });
