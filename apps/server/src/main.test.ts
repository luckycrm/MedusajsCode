...
import type { OrchestrationReadModel } from "@mctools/contracts";
...
import { CliConfig, recordStartupHeartbeat, mcCli, type CliConfigShape } from "./main";
...
const runCli = (
  ...
  env: Record<string, string> = { MCCODE_NO_BROWSER: "true" },
) => {
  return Command.runWith(mcCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      ...
      assert.equal(resolvedConfig?.baseDir, "/tmp/mc-cli-home");
      assert.equal(resolvedConfig?.stateDir, "/tmp/mc-cli-home/dev");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        MCCODE_MODE: "desktop",
        MCCODE_PORT: "4999",
        MCCODE_HOST: "100.88.10.4",
        MCCODE_HOME: "/tmp/mc-env-home",
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        MCCODE_NO_BROWSER: "true",
        MCCODE_AUTH_TOKEN: "env-token",
      });
      ...
      assert.equal(resolvedConfig?.baseDir, "/tmp/mc-env-home");
      assert.equal(resolvedConfig?.stateDir, "/tmp/mc-env-home/dev");
  ...
  const openBootstrapFd = Effect.fn(function* (payload: Record<string, unknown>) {
    ...
    const filePath = yield* fs.makeTempFileScoped({ prefix: "mc-bootstrap-", suffix: ".ndjson" });
  ...
  it.effect("uses bootstrap envelope values as fallbacks when CLI and env are absent", () =>
    Effect.gen(function* () {
      ...
      assert.equal(resolvedConfig?.baseDir, "/tmp/mc-bootstrap-home");
      assert.equal(resolvedConfig?.stateDir, "/tmp/mc-bootstrap-home/dev");
      assert.equal(resolvedConfig?.devUrl?.toString(), "http://127.0.0.1:5173/");
      assert.equal(resolvedConfig?.noBrowser, true);
  ...
  it.effect("applies CLI then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      ...
      yield* runCli(["--port", "4999", "--host", "0.0.0.0", "--auth-token", "cli-token"], {
        ...
        MCCODE_HOME: "/tmp/mc-env-home",
        MCCODE_NO_BROWSER: "true",
        MCCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
        MCCODE_LOG_WS_EVENTS: "true",
      });
  ...
  it.effect("prefers --mode over MCCODE_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over MCCODE_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        MCCODE_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/log websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        MCCODE_MODE: "desktop",
        MCCODE_LOG_WS_EVENTS: "false",
        MCCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        MCCODE_NO_BROWSER: "true",
      });
