...
import type { ContextMenuItem } from "@mctools/contracts";
import { NetService } from "@mctools/shared/Net";
import { RotatingFileSink } from "@mctools/shared/logging";
...
const BASE_DIR = process.env.MCCODE_HOME?.trim() || Path.join(OS.homedir(), ".mc");
const STATE_DIR = Path.join(BASE_DIR, "userdata");
const DESKTOP_SCHEME = "mc";
const ROOT_DIR = Path.resolve(__dirname, "../../..");
const isDevelopment = Boolean(process.env.VITE_DEV_SERVER_URL);
const APP_DISPLAY_NAME = isDevelopment ? "MedusaJS Code (Dev)" : "MedusaJS Code (Alpha)";
const APP_USER_MODEL_ID = "com.medusajs.code";
const USER_DATA_DIR_NAME = isDevelopment ? "medusajscode-dev" : "medusajscode";
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "MC Code (Dev)" : "MC Code (Alpha)";
...
function backendChildEnv(): NodeJS.ProcessEnv {
  ...
  delete env.MCCODE_PORT;
  delete env.MCCODE_AUTH_TOKEN;
  delete env.MCCODE_MODE;
  delete env.MCCODE_NO_BROWSER;
  delete env.MCCODE_HOST;
  delete env.MCCODE_DESKTOP_WS_URL;
  return env;
}
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as {
      medusajsCodeCommitHash?: unknown;
      mccodeCommitHash?: unknown;
    };
    return normalizeCommitHash(parsed.medusajsCodeCommitHash ?? parsed.mccodeCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.MCCODE_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("MedusaJS Code failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const disabledReason = getAutoUpdateDisabledReason({
    ...
    disabledByEnv: process.env.MCCODE_DISABLE_AUTO_UPDATE === "1",
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `MedusaJS Code ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
...
function showAboutDialog(): void {
  ...
  const aboutIconPath = resolveIconPath("png");
  const detailLines = [
    "MedusaJS Code is a desktop workspace for coding with AI.",
    "",
    "It combines project launchers, chat-based coding workflows, file exploration, terminal access, and git tools in one app.",
    "",
    `Version: ${app.getVersion()}`,
    `Commit: ${commitHash ?? "unknown"}`,
    "Repository: github.com/luckycrm/medusajscode",
  ];
  ...
  void dialog.showMessageBox({
    ...
    ...(aboutIconPath ? { icon: nativeImage.createFromPath(aboutIconPath) } : {}),
    buttons: ["OK"],
    noLink: true,
  });
}
    ...
    template.push({
      ...
      submenu: [
        ...
        { type: "separator" },
        {
          ...
          click: () => dispatchMenuAction("new-project"),
        },
        {
          label: "Open Folder...",
        ...
        },
        {
          ...
          click: () => dispatchMenuAction("clone-repository"),
        },
        {
          label: "Close Project",
          accelerator: "CmdOrCtrl+Shift+W",
          click: () => dispatchMenuAction("close-project"),
        },
        { type: "separator" },
        {
 ...
 * parentheses (e.g. `~/.config/MedusaJS Code (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`medusajscode`). If the legacy
...
function configureAutoUpdater(): void {
  ...
  const githubToken =
    process.env.MCCODE_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
