...
describe("store pure functions", () => {
  ...
  it("reorderProjects moves a project to a target index", () => {
    ...
    const projectThree = ProjectId.makeUnsafe("project-3");
    ...
    const next = reorderProjects(state, project1, projectThree);

    expect(next.projects.map((project) => project.id)).toEqual([project2, projectThree, project1]);
  });
});
...
describe("store read model sync", () => {
  ...
  it("preserves the current project order when syncing incoming read model updates", () => {
    ...
    expect(next.projects.map((project) => project.id)).toEqual([project2, project1, projectThree]);
  });
});
