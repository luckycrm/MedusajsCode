...
} from "@mctools/contracts/settings";
...
export default function Sidebar() {
  ...
  const activeProjectIdValue = activeProjectId ? String(activeProjectId) : null;
  const shouldShowActiveProjectOnly = routeThreadId !== null;
  const visibleProjects = useMemo(() => {
    if (!shouldShowActiveProjectOnly || !activeProjectIdValue) {
      return sortedProjects;
    }

    return sortedProjects.filter((project) => String(project.id) === activeProjectIdValue);
  }, [activeProjectIdValue, shouldShowActiveProjectOnly, sortedProjects]);
  ...
  const activeProjectThreads = useMemo(() => {
    ...
    return sortThreadsForSidebar(
      threads.filter((thread) => String(thread.projectId) === activeProjectIdValue),
      appSettings.sidebarThreadSortOrder,
    );
  ...
  function renderChatListOnly(projectThreads: readonly Thread[]) {
    ...
    const renderThreadRow = (thread: Thread) => {
      ...
      return (
        <SidebarMenuItem key={thread.id} className="rounded-md" data-thread-item>
          <SidebarMenuButton
    ...
    return <>{projectThreads.map((thread) => renderThreadRow(thread))}</>;
  }
        ...
        <SidebarGroup className="px-2 py-2">
          <div className="mb-1 flex items-center justify-between px-2">
            <span className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground/60">
              {shouldShowActiveProjectOnly ? "Chats" : "Projects"}
            </span>
          ...
          {shouldShowProjectPathEntry && !shouldShowActiveProjectOnly && (
          ...
          {shouldShowActiveProjectOnly ? (
            <SidebarMenu ref={attachProjectListAutoAnimateRef}>
              {activeProjectThreads.length > 0 ? renderChatListOnly(activeProjectThreads) : null}
            </SidebarMenu>
          ...
          {shouldShowActiveProjectOnly && activeProjectThreads.length === 0 && (
