...
import { DEFAULT_CLIENT_SETTINGS } from "@mctools/contracts/settings";
...
function createBaseServerConfig(): ServerConfig {
  return {
    cwd: "/repo/project",
    keybindingsConfigPath: "/repo/project/.medusajscode-keybindings.json",
    keybindings: [],
    issues: [],
    providers: [
      {
...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("medusajscode:last-editor", "vscodium");
    setDraftThreadWithoutWorktree();
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            cwd: "/repo/project",
            env: {
              MCCODE_PROJECT_ROOT: "/repo/project",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              MCCODE_PROJECT_ROOT: "/repo/project",
              MCCODE_WORKTREE_PATH: "/repo/worktrees/feature-draft",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
        ...
        if (body._tag === WS_METHODS.gitResolvePullRequest) {
          return {
            pullRequest: {
              ...
              url: "https://github.com/luckycrm/medusajscode/pull/1359",
              baseBranch: "main",
      ...
      await vi.waitFor(
        () => {
          ...
          expect(openRequest).toMatchObject({
            ...
            env: {
              MCCODE_PROJECT_ROOT: "/repo/project",
              MCCODE_WORKTREE_PATH: "/repo/worktrees/pr-1359",
            },
          });
        },
        { timeout: 8_000, interval: 16 },
