...
import { Config, Data, Effect, Exit, FileSystem, Layer, Logger, Option, Path, Schema } from "effect";
...
interface StagePackageJson {
  ...
  readonly mccodeCommitHash: string;
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "MCCODE_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("MCCODE_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "MCCODE_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("MCCODE_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("MCCODE_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("MCCODE_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("MCCODE_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("MCCODE_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("MCCODE_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
});
...
function stageMacIcons(stageResourcesDir: string, verbose: boolean) {
  return Effect.gen(function* () {
    ...
    const iconBuildExit = yield* Effect.exit(
      generateMacIconSet(iconSource, iconIcnsPath, tmpRoot, path, verbose),
    );
    if (Exit.isFailure(iconBuildExit)) {
      if (yield* fs.exists(iconIcnsPath)) {
        yield* Effect.logWarning(
          `[desktop-artifact] Failed to regenerate icon.icns, reusing staged icon.icns.`,
        );
        return;
      }
      return yield* new BuildScriptError({
        message: "Failed to generate macOS icon.icns and no staged fallback was available.",
      });
    }
  });
}
...
function resolveGitHubPublishConfig():
  ...
  | undefined {
  const rawRepo =
    process.env.MCCODE_DESKTOP_UPDATE_REPOSITORY?.trim() ||
...
const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    appId: "com.mctools.mccode",
    productName,
    artifactName: "MC-Code-${version}-${arch}.${ext}",
    directories: {
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `mccode-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    name: "mc-code-desktop",
    version: appVersion,
    buildVersion: appVersion,
    mccodeCommitHash: commitHash,
    private: true,
    description: "MedusaJS Code desktop build",
    author: "MedusaJS",
    main: "apps/desktop/dist-electron/main.js",
    build: yield* createBuildConfig(
      options.platform,
      options.target,
      desktopPackageJson.productName ?? "MedusaJS Code",
      options.signed,
    ),
    dependencies: {
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: MCCODE_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: MCCODE_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: MCCODE_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: MCCODE_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: MCCODE_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `bun run build:desktop` and use existing dist artifacts (env: MCCODE_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: MCCODE_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: MCCODE_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: MCCODE_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for MedusaJS Code."),
