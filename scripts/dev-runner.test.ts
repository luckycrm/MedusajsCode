...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit MCCODE_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "MCCODE_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid MCCODE_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults MCCODE_HOME to ~/.mc when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.MCCODE_HOME, resolve(homedir(), ".mc"));
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.MCCODE_HOME, resolve("/tmp/custom-mc"));
        assert.equal(env.MCCODE_PORT, "4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.MCCODE_NO_BROWSER, "1");
        assert.equal(env.MCCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.MCCODE_LOG_WS_EVENTS, "1");
        assert.equal(env.MCCODE_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            MCCODE_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          mcHome: undefined,
        ...
        assert.equal(env.MCCODE_MODE, "web");
        assert.equal(env.MCCODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.MCCODE_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom mcHome when provided", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {},
        ...
        assert.equal(env.MCCODE_HOME, resolve("/tmp/my-mc"));
      }),
    );
    ...
    it.effect("does not export backend bootstrap env for dev:desktop", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.MCCODE_PORT, undefined);
        assert.equal(env.MCCODE_AUTH_TOKEN, undefined);
        assert.equal(env.MCCODE_MODE, undefined);
        assert.equal(env.MCCODE_NO_BROWSER, undefined);
        assert.equal(env.MCCODE_HOST, undefined);
