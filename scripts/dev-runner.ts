...
import { NetService } from "@mctools/shared/Net";
...
export const DEFAULT_MC_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(homedir(), ".mc"),
);
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("MCCODE_PORT_OFFSET"),
  devInstance: optionalStringConfig("MCCODE_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid MCCODE_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `MCCODE_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric MCCODE_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed MCCODE_DEV_INSTANCE=${seed}` };
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly mcHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(mcHome);
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      MCCODE_HOME: resolvedBaseDir,
    };

    if (!isDesktopMode) {
      output.MCCODE_PORT = String(serverPort);
      output.VITE_WS_URL = `ws://localhost:${serverPort}`;
    } else {
      delete output.MCCODE_PORT;
      delete output.VITE_WS_URL;
      delete output.MCCODE_AUTH_TOKEN;
      delete output.MCCODE_MODE;
      delete output.MCCODE_NO_BROWSER;
      delete output.MCCODE_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.MCCODE_HOST = host;
    }

    if (!isDesktopMode && authToken !== undefined) {
      output.MCCODE_AUTH_TOKEN = authToken;
    } else if (!isDesktopMode) {
      delete output.MCCODE_AUTH_TOKEN;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.MCCODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.MCCODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.MCCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.MCCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.MCCODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.MCCODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.MCCODE_MODE = "web";
      delete output.MCCODE_DESKTOP_WS_URL;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read MCCODE_PORT_OFFSET/MCCODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("MCCODE_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv("MCCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD"),
      logWebSocketEvents: readOptionalBooleanEnv("MCCODE_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.MCCODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.MCCODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  mcHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all MedusaJS Code data (equivalent to MCCODE_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("MCCODE_HOME")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to MCCODE_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("MCCODE_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to MCCODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("MCCODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to MCCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("MCCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to MCCODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("MCCODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to MCCODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("MCCODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to MCCODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("MCCODE_PORT")),
  ),
