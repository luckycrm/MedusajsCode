...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `mc` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@mctools/shared/git`, `@mctools/shared/DrainableWorker`) — no barrel index.
