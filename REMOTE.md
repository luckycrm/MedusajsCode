...
Use this when you want to open MedusaJS Code from another device (phone, tablet, another laptop).

## CLI ↔ Env option map

The MedusaJS Code CLI accepts the following configuration options, available either as CLI flags or environment variables:

| CLI flag                | Env var               | Notes                                                                                |
| ----------------------- | --------------------- | ------------------------------------------------------------------------------------ |
| `--mode <web\|desktop>` | `MCCODE_MODE`         | Runtime mode.                                                                        |
| `--port <number>`       | `MCCODE_PORT`         | HTTP/WebSocket port.                                                                 |
| `--host <address>`      | `MCCODE_HOST`         | Bind interface/address.                                                              |
| `--base-dir <path>`     | `MCCODE_HOME`         | Base directory.                                                                      |
| `--dev-url <url>`       | `VITE_DEV_SERVER_URL` | Dev web URL redirect/proxy target.                                                   |
| `--no-browser`          | `MCCODE_NO_BROWSER`   | Disable auto-open browser.                                                           |
| `--auth-token <token>`  | `MCCODE_AUTH_TOKEN`   | WebSocket auth token. Use this for standard CLI and remote-server flows.             |
| `--bootstrap-fd <fd>`   | `MCCODE_BOOTSTRAP_FD` | Read a one-shot bootstrap envelope from an inherited file descriptor during startup. |
